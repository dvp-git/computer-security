...
def per_byte_sign(size, sign_time):
    return sign_time / size 

def per_byte_verify(size, verify_time):
    return verify_time / size
...
def DSA_(key_size , file_name):
    ...
    return f"Filename: {file_name}:{size}  , Keysize:{key_size} bits, KeyGen Time :{key_gen_time} ns, Sign Time: {sign_time} ns, Verify Time: {verify_time} ns, Per Byte Sign time: {perbyte_sign} ns, Per Byte Verify time: {perbyte_ver} ns"
